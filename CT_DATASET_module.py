...
class CT_DATASET:
    ...
    def _read_dicom_images(self,path_of_dicom_slices_from_one_patient,folder_of_patient_name):
        ...
        if sex!="M" or sex!="F":
            # sex = "M" if str(name.original_string).split('b')[-1].split("^")[0][-1]=='S' else "F"
            sex = (
                    "M"
                    if str(name.original_string).split('b')[-1].split("^")[1][-1] == 'S'
                    else "M"
                    if str(name.original_string).split('b')[-1].split("^")[1][-1] == 'L'
                    else "M"
                    if str(name.original_string).split('b')[-1].split("^")[1][-1] == 'N'
                    else "F"
                )
...
###########################################################################################################
        ...
        if midline(mask[central_slice],name,graph=False)<-14:
            ...
            print("patient's name:{},patient's sex:{},patients'age:{},Glasgow Coma Scale:{}".format(name,sex,age,GCS))
            return {'volume':volume_aligned,
            'name':name,
            'sex':sex,
            'age':age,
            'Glasgow Coma Scale' : GCS
