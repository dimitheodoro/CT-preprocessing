...
###########################################################################################################
        ...
        if midline(mask[central_slice],name,graph=False)<-14:
            ...
            print(min_max_normalize(volume_aligned).min(),min_max_normalize(volume_aligned).max())
            return {'volume':min_max_normalize(volume_aligned),
            'name':name,
            'sex':sex,
            'age':age,
            'Glasgow Coma Scale' : GCS,
        ...
        print("patient's name:{},patient's sex:{},patients'age:{},Glasgow Coma Scale:{},Class:{}".format(name,sex,age,GCS,CLASS))
        print("-"*100)
        print(min_max_normalize(volume).min(),min_max_normalize(volume).max())
        return {'volume':min_max_normalize(volume),
                'name':name,
                'sex':sex,
                'age':age,
                'Glasgow Coma Scale' : GCS,
